...
$queryTotal = "SELECT COUNT(*) quant FROM TRNG  WHERE company = '".$_SESSION["company"]."'";
...
$query = "SELECT T2.category, T1.title, T1.file, T1.file link, T1.remarks, T1.id FROM TRNG T1 JOIN TCAT T2 ON T1.category = T2.id WHERE T1.company = '".$_SESSION["company"]."'";
...
while($row = mysql_fetch_assoc($result)) {
	...
	$nestedData[] = $row["file"];
	$nestedData[] = "<a href='ftp/herramientas/".utf8_encode($row["link"])."' target='_blank'><i class='fa fa-external-link' aria-hidden='true'></i></a>";
