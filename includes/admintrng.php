...
if(move_uploaded_file($_FILES['file']['tmp_name'],$upload_path . $filename)) {
	...
	$query = "INSERT INTO TRNG (id, title, category, file, active, emp_id, created_at, remarks, company) VALUES ('', '$title', '$category', '$filename', 'Y', ".$_SESSION["salesPrson"].", now(), '$remarks', '".$_SESSION["company"]."')";
	$result = mysql_query($query);
	
	if(!$result) {
		die('Could not enter data Tool: ' . mysql_error());
	}
	header('Location: /docscapacitacion.php');
