var itemLine = "<div class='item'>\
               ...
               <div class='itemRight'>\
                ...
                <div class='batch-div' style='display: none;'>\
				  <button type='button'>Ver Lotes</button>\
				  <div>\
				    <table>\
				  </div>\
				</div>\
		...
		$.ajax({
			...
			success: function(prodDetail){
				...
				foreach($whCodes as $whCode) {
					...
					echo "if(prodDetail['e".$whCode."'] == -1){";
						echo "$('.oH".$whCode.":eq('+ind+')').html(\"<i class='fa fa-ban' aria-hidden='true'></i>\");";
						echo "totOnHand += 0;";
						echo "$('.oO".$whCode.":eq('+ind+')').find('span').html(\"<i class='fa fa-ban' aria-hidden='true'></i>\");";
						echo "$('.oO".$whCode.":eq('+ind+')').find('.divOOSD').html(\"<i class='fa fa-ban' aria-hidden='true'></i>\");";
					//}
					echo "}else{";
					//else{
						echo "prodDetail['qu".$whCode."'] == null ? $('.batch".$whCode.":eq('+ind+')').attr('disabled', 'disabled') : $('.batch".$whCode.":eq('+ind+')').removeAttr('disabled');";
						echo "$('.batch".$whCode.":eq('+ind+')').on('click', function(){";
						echo "console.log($('.batch-div table'));";
						echo "$('.batch-div table').empty();";
						echo "$('.batch-div table').append($('<tr><th>ID Lote</th><th>Cantidad</th></tr><tr><td>' + prodDetail['bn".$whCode."'] + '</td><td>' + prodDetail['qu".$whCode."'] + '</td></tr>'));";
						echo "$('.batch-div table').show()";
						echo "});";
