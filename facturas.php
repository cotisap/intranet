...
function GridRowCount() {
	...
	$('.dataTables_wrapper').find('.dataTables_filter').append("<button type='button' class='expandedGroup clpsXpndBT collapsed green'>Expandir todo</button>");
	$('.dataTables_wrapper').find('.dataTables_filter').append("<button type='button' class='collapsedGroup clpsXpndBT expanded red'>Colapsar todo</button>");
...
<p class='reportTitle'>Facturas</p>
...
foreach ($salesPerson as $sp) {
	$query = "SELECT NumAtCard, DocNum, (CardName + ' - ' + CardCode) AS Cliente, DocDate, SlpCode, DocTotal FROM OINV
WHERE SlpCode = $sp
...
ORDER BY DocDate DESC";
	...
	echo "<div class='reportContainer'><table class='reportTable' id='".$sp."'><thead><tr><th>Cliente</th><th>Folio pedido</th><th>No. de documento</th><th>Fecha</th><th class='h0' width='160px'>Monto total</th><th width='40px'></th></tr></thead><tfoot><tr><th></th><th></th><th></th><th>Fecha</th><th class='cd0'></th><th></th></tr></tfoot><tbody>";
	...
	while($row = mssql_fetch_array($result)) {
	  ...
	  echo "<tr><td>".$row["Cliente"]."</td><td>".$row["NumAtCard"]."</td><td>".$row["DocNum"]."</td><td>".date_format($cotDate, "d/m/Y")."</td><td class='d0'>".number_format($tot, 2)."</td><td><a href='verfactura.php?num=".$row["DocNum"]."' class='viewDetails'><img src='images/details.png'></a></td></tr>";
	}
