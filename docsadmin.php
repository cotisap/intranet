...
$(document).ready( function () {
	...
	var table = $('#docsList').DataTable({
		"aoColumnDefs": [
			...
			{'className': 'dt-center', 'aTargets': [6,7] }//,
	...
	$('#docsList tbody').on('click', 'tr a.updateDoc', function () {
    	...
    	$(datosRow[0].children[3]).html("<input type='text' data-initial='"+ 
    										datosRow[0].children[3].innerText +"' value='"+ 
    											datosRow[0].children[3].innerText +"'>");
    	
    	$(datosRow[0].children[6]).html("<img class='guardar' data-id='"+ dataId +"' src='images/icons/save.png'>" 
	...
	$('#docsList tbody').on('click', 'tr img.cancelar', function () {
		...
		$(datosRow[0].children[6]).html("<a data-id='" + dataId + "' class='updateDoc'><img src='images/pencil.png'></a>");
		$(datosRow[0].children[2]).text($(datosRow[0].children[2].children[0]).attr('data-initial'));
		$(datosRow[0].children[3]).text($(datosRow[0].children[3].children[0]).attr('data-initial'));
	...
	$('#docsList tbody').on('click', 'tr img.guardar', function () {
		...
		console.log($(datosRow[0].children[3].children[0]).val());

		$.ajax({
			async: false,
			type: 'post',
			url: 'includes/ajaxToolsUpdate.php',
			data: {
				id : dataId,
				titulo : $(datosRow[0].children[2].children[0]).val(),
				descripcion : $(datosRow[0].children[3].children[0]).val()
			},
			success: function() {
					$(datosRow[0].children[2]).text($(datosRow[0].children[2].children[0]).val());
					$(datosRow[0].children[3]).text($(datosRow[0].children[3].children[0]).val());
			},
			error: function () {
	...
	$('#docsList tbody').on('click', 'tr a.removeDoc', function () {
    	...
    	$.ajax({
    		type: 'post',
    		url: 'includes/ajaxToolsDelete.php',
    		data: {
    			id : dataId,
    			filename : datosRow[0].children[4].innerText
    			},
