...
<table class="formTable">
	<tr>
    	<td align="left" class="pageTitle">Pedidos</td>
        <td align="right">
...
if ($_SESSION["admin"] == 'Y') {
	...
	$salesPerson = [$_SESSION["salesPrson"]];
}
	
foreach ($salesPerson as $sp) {
	$query = "SELECT NumAtCard, (CardName + ' - ' + CardCode) AS Cliente, DocDate, SlpCode, DocTotal FROM ORDR
WHERE SlpCode = $sp
AND DocStatus = 'O'
ORDER BY DocDate DESC";
	...
	echo "<div class='reportContainer'><table class='reportTable' id='".$sp."'><thead><tr><th>Cliente</th><th>Folio pedido</th><th>Fecha</th><th class='h0' width='160px'>Monto total</th><th width='40px'></th></tr></thead><tfoot><tr><th></th><th></th><th>Fecha</th><th class='cd0'></th><th></th></tr></tfoot><tbody>";
	...
	while($row = mssql_fetch_array($result)) {
	  ...
	  echo "<tr><td>".$row["Cliente"]."</td><td>".$row["NumAtCard"]."</td><td>".date_format($cotDate, "d/m/Y")."</td><td class='d0'>".number_format($tot, 2, ".", ",")."</td><td><!--<a href='verpedido.php?idSO=".$row["NumAtCard"]."' class='viewDetails'><img src='images/details.png'></a>--></td></tr>";
	}
