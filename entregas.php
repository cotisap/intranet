...
function GridRowCount() {
	...
	$('.dataTables_wrapper').find('[id|=group-id]').each(function () {
		...
		for (var i=0; i<ranges.length; i++) {
			...
			$(this).find(".bp" + ranges[i]).html(localeString(bpSum));
		}
	});
	getTotSums();
...
foreach ($salesPerson as $sp) {
	...
	echo "<div class='reportContainer'><table class='reportTable' id='".$sp."'><thead><tr><th>Cliente</th><th>Folio entrega</th><th>Folio transacci&oacute;n</th><th>Fecha</th><th class='h0' width='160px'>Monto total</th><th width='40px'></th></tr></thead><tfoot><tr><th></th><th></th><th></th><th>Fecha</th><th class='cd0'></th><th></th></tr></tfoot><tbody>";
	...
	while($row = mssql_fetch_array($result)) {
	  ...
	  echo "<tr><td>".$row["Cliente"]."</td><td>".$row["DocNum"]."</td><td>".$row["NumAtCard"]."</td><td>".date_format($cotDate, "d/m/Y")."</td><td class='d0'>".number_format($tot, 2)."</td><td><a href='verEntrega.php?num=".$row["DocNum"]."' class='viewDetails'><img src='images/details.png'></a></td></tr>";
	}
...
function getTotSums() {
	...
	for (var t=0; t<tableIds.length; t++) {
		...
		$.each( ranges, function( i, val ) {
			...
			$('#'+tableIds[t]+' .cd' + val).html(localeString(sum));
		});
	}
}
</script>
