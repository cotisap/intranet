...
$queryQuote = "SELECT Codigo_SN, CardName, Serie, DiscPrcnt, tax, Total_MN, Total_USD, Comentarios, Empl_Ven, Id_Cot1, DocNum, TC, account, cnotes, dlvType, dlvPerson, dlvPhone, dlvEmail, dlvAddress, dlvFlet, company, DocCur, status, st, razon, otro FROM COTI WHERE Id_Cot1 = '$idCot'";
...
<table class="formTable">
  <tbody>
    <tr>
        ...
        <!-- @Author: Sergio Mireles && Gerardo Steven | Inicio -->    
        <tr>
            <td>
                
                <div class="status">
	                <span>Estatus de la cotizaci&oacute;n</span>
	                    <select id="status">
	                    ...
	                    <select id="reason" style="display: none;" ">
                            ...
                            <option value="1">Por stock</option>
                            <option value="2">Por seguimiento</option>
	                    </select>
	                    <br/>
	                    <textarea id="other" rows="5" cols="40" style="display: none;" placeholder="Escribe la razon por la cual se perdio"></textarea>
	                <a class="button green" id="btn-st">Actualizar estatus</a>    
                </div>
                ...
                <script type="text/javascript">
                    $(document).ready(function () {
                        ...
                        var stval = new Array('Cotizada',
                                              'Negociando',
                                              'Ganada',
                                              'Perdida');

                        $("select#status option:selected").text(stval[st-1]);
                        $("select#status option:selected").val(stval[st-1]);
                        ...
                        $.each(stval, function (key, value) {
                            $("select#status option:selected").after(
                        ...
                        $("#btn-st").on("click", function () {
                        	...
                        	var razon = status == "Perdida" ? $("select#reason option:selected").text() : "";
                        	var otro = $("select#reason option:selected").val() == "3" ? $("#other").val() : "";
                            $.ajax({
                                method: "post",
                                url: "includes/updateST.php",
                                data: {
                                    id: <?php echo $idCot; ?> ,
                                    st: status,
                                    "razon" : razon,
                                ...
                                error: function () {
                                    alert("Se genero un error durante la Transferencia")
                                }
                            });
                        });
    	...
    	<td colspan="2">
        	<ul class="buttonBar">
               ...
               <?php
                ...
                <li><button type="button" class="button sYellow" id="showSAP" disabled><i class="fa fa-window-restore" aria-hidden="true"></i> Crear en SAP</button></li>
								...
								<tr>\
									<td class='tdMethod'>\
										<select id='pMethod' name='pMethod[]' class='pMethod' required>\
											...
											<option value='03'>Transferencia electr&oacute;nica</option>\
...
$(document).on("click", "#createSAP", function () {
	...
	$("#quote").attr("action", "http://corporativo-vallejo-tntchvdtpq.dynamic-m.com:8380/Default.aspx");
	//$("#quote").attr("action", "http://mbr-nznqgdqbdc.dynamic-m.com:8380/Default.aspx");
	$("#quote").submit();
});
...
$queryPayments = "SELECT T1.ref_q, T3.text AS method, T1.ammount, T1.currency, T1.ref_p, T1.date, T1.file, T2.text AS status FROM PMNT T1 INNER JOIN PSTS T2 ON T1.status = T2.val INNER JOIN PMTD T3 ON T1.method = T3.val WHERE ref_q = '$idCot'";
$resultPayments = mysql_query($queryPayments);
...
$files = Array();
$status = Array();
while($rowPayments = mysql_fetch_assoc($resultPayments)){ 
...
$("#DocCur, .currency, .discount, .uniMed, #comNots, #account, #btCancel, #saveQuote, #showSAP, #ivaP, #dlvType").prop("disabled", true);
